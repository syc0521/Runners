			...
			if (clipAsset != null) {
				if (clipAsset.guid != m_lastClipId) {
					...
					if (clipAsset.m_movieData != null) {
						...
						m_boundMovieMaterial.player.SetData(clipAsset.gcHandle.AddrOfPinnedObject(), clipAsset.m_movieData.bytes.LongLength);
		...
		private async void ProcessFrameOnSeeking(TimelineClip activeClip, CriManaClip clip, double frameTime) {
			...
			if (clip.m_movieData != null) {
				...
				if (clip.gcHandle.Target != null) {
					m_boundMovieMaterial.player.SetData(clip.gcHandle.AddrOfPinnedObject(), clip.m_movieData.bytes.LongLength);
				} else {
					/* empty clip */
					return;
				}
			} else if (string.IsNullOrEmpty(clip.m_moviePath) == false) {
				m_boundMovieMaterial.player.SetFile(null, clip.m_moviePath);
			} else {
				return;
			}
